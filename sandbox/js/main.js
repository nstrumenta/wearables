...
window.addEventListener("message", (e) => {
  ...
  switch (e.data.type) {
    ...
    case 'loadFile':
      ...
      inputEvents = [];
      const lines = e.data.payload.split(/[\r\n]+/g);
      console.log('rows:' + lines.length);
      for (var i = 0; i < lines.length; i++) {
        var event = SensorEventUtils.eventFromText('nst-events.ldjson', lines[i]);
        if (event) {
          inputEvents.push(event);
        }
      }

      inputEvents = inputEvents.sort(function (a, b) {
        return a.timestamp - b.timestamp
      });
      idbKeyval.set(idbKey + '/inputEvents', JSON.stringify(inputEvents))
      renderInputEvents();
      setControllerRanges();
      sendMessageToParent('loaded file');

      // storeCompressedJson('nst-input-events', inputEvents);
      break;
    case 'loadAlgorithm':
      algorithmWorker = new Worker('js/algorithmWorker.js');
      algorithmWorker.onmessage = function (e) {
        console.log('message from algorithmWorker', e.data);
        switch (e.data.type) {
          case 'runComplete':
            idbKeyval.get(e.data.payload).then((val) => {
              outputEvents = JSON.parse(val);
              console.log('outputEvents.length = ', outputEvents.length);
              clearPolylines();
              renderOutputEvents();
              onRunComplete();
            });
            break;
          default:
            sendMessageToParent(e.data);
            break;
        }
      }
      algorithmWorker.postMessage(e.data);
      break;
    case 'centerMap':
      centerMap();
      break;
...
var deviceMotionListener = false;
function handleMotionEvent(event) {
  // console.log(event)
  const timestamp = Date.now();
  const accel = [event.accelerationIncludingGravity.x, event.accelerationIncludingGravity.y, event.accelerationIncludingGravity.z];
  const gyro = [event.rotationRate.x, event.rotationRate.y, event.rotationRate.z];
...
async function recordDeviceMotion() {
  nstrumenta.init();
  window.addEventListener("devicemotion", handleMotionEvent);
  deviceMotionListener = true;
}
...
; var Parameters = function () {
  ...
  this.toggleDeviceMotion = function (options) {
    console.log('toggleDeviceMotion', deviceMotionListener);
    if (!deviceMotionListener) {
      console.log('starting recordDeviceMotion');
      controllers.recordDeviceMotionButton.name("Stop Recording");
      recordDeviceMotion();
    } else {
      console.log('stopping recordDeviceMotion');
      controllers.recordDeviceMotionButton.name("Record DeviceMotion");
...
controllers.recordDeviceMotionButton = gui.add(parameters, 'toggleDeviceMotion').name('Record DeviceMotion');
