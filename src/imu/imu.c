...
{
    imu_data->mag_sample = vec3(event.values[0], event.values[1], event.values[2]);
...
void imu_update_gyro(imu_data_t *imu_data, nst_event_t event, nst_event_t *output_events, int *output_events_count)
{
    ...
    // location_update_accelerometer(imu_data, acc_event, output_events, output_events_count);

    // location_update_magnetic_field(imu_data, mag_event, output_events, output_events_count);

    // location_update_gyro(imu_data, gyro_event, output_events, output_events_count);
